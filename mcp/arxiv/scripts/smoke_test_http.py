#!/usr/bin/env python3
"""Smoke test for a running arxiv-mcp: MCP handshake + tool calls over
streamable-HTTP.

Run with the same Python that has `mcp` installed (the release venv), e.g.:
  <deploy-root>/current/.venv/bin/python scripts/smoke_test_http.py http://host:8003

Usage:
  smoke_test_http.py [base-url]   (default: http://127.0.0.1:8003)
"""
    ...
    async with streamable_http_client(url) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            result = await session.call_tool(
                "search_papers", {"category": "hep-ex", "max_results": 1}
            )
            text = result.content[0].text if result.content else ""
            print(f"OK: search_papers(category=hep-ex, max_results=1) -> {text}")
