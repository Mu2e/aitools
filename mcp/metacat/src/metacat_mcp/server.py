...
# / dqm-mcp pattern) rather than inside a factory function, so tools are
# decorated once at import time.
mcp = MCPServer("metacat", instructions=READ_ONLY_INSTRUCTIONS)
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="metacat-mcp", description=__doc__)
    parser.add_argument(
        "--host",
        default=os.environ.get("METACAT_MCP_HOST", DEFAULT_HOST),
        help="bind address (default: %(default)s)",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.environ.get("METACAT_MCP_PORT", str(DEFAULT_PORT))),
...
def main(argv: list[str] | None = None) -> None:
    ...
    LOGGER.info("Starting metacat MCP server over streamable-http on %s:%s", args.host, args.port)
