...
def _wrap(fn):
    ...
    return a list (list_runs, get_cidtables): the SDK converts a
    successful return value against the tool's declared output schema
    *after* the function returns, and a dict doesn't satisfy a
    list[...]-typed schema -- confirmed live, the conversion failure gets
    classified as an unexpected crash, and per the SDK's own design that
    deliberately withholds the real message from the caller ("Error
    executing tool <name>", nothing else). Raising ToolError instead
    sidesteps output conversion entirely and is exactly the mechanism the
    SDK provides for "a failure you saw coming": the message reaches the
    caller as an is_error=True result, logged server-side at INFO without
    a traceback -- so the LOGGER.exception call below is kept, to still
    get a full traceback in this server's own logs for the genuinely
    unexpected case.
    """
